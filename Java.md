...
| `int`     | `4`     | 整数除以`0`会抛出错误                                        |
| `long`    | `8`     |                                                              |
| `char`    | `2`     | 可以表示`unicode`字符，通过使用`\u####`指定字符              |
| `float`   | `4`     | `0/0`产生`NaN`，`+/0`产生`Infinity`，`-/0`产生`-Infinity`    |
| `double`  | `8`     | `0/0`产生`NaN`，`+/0`产生`Infinity`，`-/0`产生`-Infinity`    |
| `boolean` | `1/4`  | 单个布尔元素占用`4B`，布尔数组中单个布尔元素占用`1B`         |
...
+ `method`：
  ...
  + `final`修饰的`method`不能被派生类`Override`
  + `static`方法只能访问静态成员
+ `constructor`
